...
const CHAT_SYSTEM_PROMPT = `You are a helpful customs classification assistant specializing in Thailand AHTN 2022 HS codes. You have access to the classification result and can answer follow-up questions.
...
${context.alternatives?.length ? `ALTERNATIVES CONSIDERED:
${context.alternatives.map(a => `- ${a.hs_code}: ${a.description} (${a.reason})`).join('\n')}` : ''}

${context.edge_cases?.length ? `EDGE CASES:
${context.edge_cases.map(e => `- ${e.hs_code}: ${e.condition}`).join('\n')}` : ''}`;
    ...
    const messages: Array<{ role: "system" | "user" | "assistant"; content: string }> = [
      {
        role: "system",
        content: CHAT_SYSTEM_PROMPT + "\n" + classificationContext + additionalContext,
      },
    ];
